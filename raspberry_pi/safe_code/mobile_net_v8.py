...
# ===== Gemini API Integration using Shared Memory =====
class GeminiAPI:
    ...
    def send_to_api(self, image_data):
        """Send image to Gemini API and handle MP3 response"""
        try:
            ...
            if response.status_code == 200:
                ...
                if 'audio' in content_type or 'mp3' in content_type:
                    print("🎵 Received MP3 audio response")
    ...
    def play_audio_response(self, audio_data):
        """Play MP3 audio response"""
        try:
            # Save MP3 data to temporary file
            temp_audio_file = "temp_gemini_response.mp3"
    ...
    def process_image(self):
        """Complete Gemini processing workflow using shared memory"""
        print("🤖 Processing image from shared memory for Gemini analysis...")
        ...
        if image_data:
            ...
            print("❌ Failed to capture image from shared memory")
