...
class SIFTObjectTracker:
    def __init__(self):
        ...
        self.end_point = None
        self.min_matches = 10
        self.MOVE_THRESHOLD = 10  # Threshold in pixels for significant movement
    ...
    def add_tracker(self, bbox, frame):
        ...
        if template_desc is not None and len(template_desc) > self.min_matches:
            tracker = {
                ...
                'prev_center': None,  # To store previous center X
                'lost': False         # To mark if object is out of frame
            }
        ...
        while True:
            ...
            for tracker in self.trackers:
                ...
                if result is not None:
                    ...
                    prev_center = tracker.get('prev_center')
                    
                    # Only print direction if movement exceeds threshold
                    if prev_center is not None:
                        movement = center_x - prev_center
                        if abs(movement) < self.MOVE_THRESHOLD:
                            print("Object is not moving.")
                        elif movement > 0:
                            print("Object moving right")
                        elif movement < 0:
                            print("Object moving left")
                    tracker['prev_center'] = center_x

                    # Detect if left the screen (left/right)
                    if center_x < 0:
                        print("Object went out of screen from left side!")
                        tracker['lost'] = True
                    elif center_x > frame_w:
                        print("Object went out of screen from right side!")
                        tracker['lost'] = True
                else:
                    pass
